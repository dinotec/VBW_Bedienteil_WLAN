...
class enum_file {
	...
	public:
		...
		void operator()(string s) {
			...
			if(x1 != string::npos) {
				...
				//~ while(isalpha(s[x1]) || isdigit(s[x1]) || s[x1]=='_')
						//~ sx += s[x1++];
				vector<string>::iterator p = find(str.begin(), str.end(), sx);
				if(p == str.end()) {
					str.push_back(sx);
				}	
			}
		...
		operator vector<string>() {
			vector<string> ret;
			for(vector<string>::iterator p = str.begin(); p != str.end(); ++p) {
				ostringstream os;
...
int main (int argcs , char* pArgs[]) {
	ofstream enm_h, default_werte_h;
    ifstream in;
	int var = 0;
	if(argcs == 2)
		in.open (pArgs[1]);
	...
	if(in) {
		enm_h.open("enum.h");
		default_werte_h.open("default_werte.h");

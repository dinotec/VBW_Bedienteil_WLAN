...
void display_flush_pcd5(bit invt) { // 1 oder 2 Display normal oder umdrehen Bild invertieren
	...
	for(seg = 0; seg < 8; ++seg) {
		...
		_nop_();
		_nop_();
		for(zlr = 0; zlr < WIDTH; ++zlr) {
			send(1, rram.vec[seg][zlr]);
			WDT_RT();
		}
	}
	send(0, 0xE8);    // bias=1/10.7
	send(0, 0xC4);    // Set map control: Com1->Com128; SEG240->SEG1; C1~C8=>D7~D0
	send(0, 0x23); 	  // Set duty to be 1/96, temperature compensation -0.2%
	send(0, 0x81); 	  // Set gain and potentionmeter_(1) 
	//~ send(0, 0xA0); delay_dsp();	  // Set gain and potentionmeter_(2)	GN=1, PM=34
	//~ send(0, 0x78); 	  // Set gain and potentionmeter_(2)	GN=1, PM=34
	{
		xdata unsigned char k;
		k = vek[contr].b[1] + 20; 
		if(k < 64)
			send(0, 0x40 | k); 	  // Set gain and potentionmeter_(2)	GN=1, PM=34
		else	
			send(0, 0x80 | (k - 34)); 	  // Set gain and potentionmeter_(2)	GN=1, PM=34
	}
	send(0, 0x2F); 	// Use internal VLCD and panel load: 60nF~90nF

	send(0, 0x89); 	// Set RAM address control
	send(0, 0x40); 	// set Start line 

	send(0, 0xA4); 	// Normal dispaly mode
	send(0, 0xAf); 	// display on
 ...
 void lcdInit(void) {
	...
	delay_dsp();
	RESET(1);
	send(0, 0xE2); delay_dsp();   // Soft reset uc1608
	send(0, 0xE8); delay_dsp();   // bias=1/10.7
	send(0, 0xC4); delay_dsp();   // Set map control: Com1->Com128; SEG240->SEG1; C1~C8=>D7~D0
	send(0, 0x23); delay_dsp();	  // Set duty to be 1/96, temperature compensation -0.2%
	send(0, 0x81); delay_dsp();	  // Set gain and potentionmeter_(1) 
	send(0, 0xA0); delay_dsp();	  // Set gain and potentionmeter_(2)	GN=1, PM=34

	send(0, 0x2F); delay_dsp();	// Use internal VLCD and panel load: 60nF~90nF

	send(0, 0x89); delay_dsp();	// Set RAM address control
	send(0, 0x40); delay_dsp();	// set Start line 

	send(0, 0xA4); delay_dsp();	// Normal dispaly mode
	send(0, 0xAf); delay_dsp();	// display on
}

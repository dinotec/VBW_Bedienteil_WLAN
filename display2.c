...
void pause_ms(unsigned char ms) {
...
void display_reset(void) {
	...
	send(0,0xe1);
	//~ -----------
	send(0,0x11); // sleep out
	send(0,0x28); // display off
	pause_ms(50);
	send(0,0xc0);//Vop=b9h
	send(1,0xb9);
	send(1,0x00);
	//~ send(0,0xc7);
	//~ send(0,0x3f );
	send(0,0xc3);// BIAS=1/9
	send(1,0x05);
	send(0,0xc4);//Booster=x8
	send(1,0x07);
	send(0,0xd0); // enable analog circuit
	send(1,0x1d);
	send(0,0xb5); // N-Line=0
	send(1,0x00);
	send(0,0x39); // monochrome mode
	send(0,0x3a); // enable DDRAM-interface
	send(1,0x02);
	send(0,0x36);
	send(1,0x00);
	send(0,0xb0); // display duty
	send(1,0x3f); // 64 Zeilen
	send(0,0xb4); // 
	send(1,0xa0); // 
	send(0,0x30); // 
	send(1,0x00); // 
	...
	send(1,0x00); // 
	send(1,0x3f); // 
	send(0,0x20); // display inversion off
	send(0,0x2a); // collumn adress setting
	send(1,0x00);
	send(1,0x00);
	send(1,0x00);
	send(1,0x4f); // 240
	send(0,0x2b); // row adress setting
	send(1,0x00);
	send(1,0x00);
	send(1,0x00);
	send(1,0x3f); // 64 zeilen
	...
	send(0,0x29); // display on
 	//~ send(0,0x23); // all pixel on
	//~ CSB = 1;
}

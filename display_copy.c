...
void display_reset(void)
{
	xdata unsigned char cnt,
						ix;
	//RESET = 0;
	// xdata unsigned char font_[8][5];
	// // unsigned char code * xdata pread;           // FLASH read pointer
	// // pread = 0xf300;
	// for(ix = 0; ix < 8; ++ix)
		// for(cnt = 0; cnt < 5; ++cnt)
			// font_[ix][cnt] = font_spec[ix][cnt];
			// //font_[cnt] = *pread++;
	...
	for(cnt = 0; cnt < 2; cnt++)
	{
		...
		send(0,0x24); // 17 internal resistor ratio set
		//send(0,0x25); // 17 internal resistor ratio set
		// send(0,0x81); // 18 electronic volume
		send(0,0xff); // 18 electronic volume
		send(0,0x1c); // 18 
		send(0,0x2f); // 16 power control set
		send(0,0xa4); // 10 display all points on/off
		send(0,0xac); // 19 static indicator off
		// send(0,0x00); // 19 static indicator on: 1 Sek. blinken
...
// void bmp_text1(unsigned char *ptr)
{
	...
	for(i = col, m = 0; m < len && i < WIDTH-5 ; i += 6, m++)
	{
		...
		{	
			//c = '#'; // c = font_spec[ptr[m] - 0x81];
			// adr_128k = EEPPROM_ADR_SZ.ul + (ptr[m] - 0x81)*5;
...
// void bmp_text2(unsigned char *ptr)
{
	xdata unsigned char x,
				  y,
				  i,m,
...
// void line_h(unsigned char *plen)
{
	xdata unsigned char page,
				  page_nr,
				  len,
...
// void line_v(unsigned char *plen)
{
	xdata unsigned char page,
				  page_nr,
				  len,
	...
	if(!MENU_MODE)
		if(vek[des].ui == pc_txt || vek[des].ui == rx_txt || (vek[des].ui==cl_txt && vek[rx_mess].ui == nein_txt)) // 136..139
		{
			bilder3();
		}		
...
void display_wr(unsigned char *ptr)
{
	...
	//"02 010 005 ABCD"
	// f  row col text
	// memset(rram.vec,0,sizeof(rram.vec));
	// strcpy(rram.vec,ptr);
	font = atoi(ptr);
	font %= 6;
	col	 = atoi(ptr + 3);	  
...
void display_flush(bit num, bit pos, bit invt) // 1 oder 2 Display normal oder umdrehen Bild invertieren
{
	...
	else
	{
		...
		{
			// send(0,0xb0 + n);	//3 page adress set 
			// send(0,0x00);	//4 column adress set low
			// send(0,0x10);	//4 column adress set high
			// A0(0);
			A0 = 0;
			...
			{
				// for(i = 1, inv = 0, k = 0x80; k > 0; i <<= 1, k >>= 1 )
				// {	
					// if(rram.vec[nn][ixx] & i) inv |= k;
				// }
				// send(1,rram.vec[nn][ixx]);
				// inv = 0;
				// if(rram.vec[nn][ixx] & 0x01) inv |= 0x80;
				// if(rram.vec[nn][ixx] & 0x02) inv |= 0x40;
				// if(rram.vec[nn][ixx] & 0x04) inv |= 0x20;
				// if(rram.vec[nn][ixx] & 0x08) inv |= 0x10;
				// if(rram.vec[nn][ixx] & 0x10) inv |= 0x08;
				// if(rram.vec[nn][ixx] & 0x20) inv |= 0x04;
				// if(rram.vec[nn][ixx] & 0x40) inv |= 0x02;
				// if(rram.vec[nn][ixx] & 0x80) inv |= 0x01;

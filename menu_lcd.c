...
void menu_ini(void) {
	...
	unsigned char xdata * xdata buf_mi;
	
	buf_mi = calloc (LEN_60, sizeof(char)); 	
	...
	//--------------------------------------------------------
				...
				read_text(buf_mi);
				strcat(txt_dflt[vbw_typ], buf_mi);
...
void zeile_fuellen(unsigned char zeile, unsigned int text_nr) {
	...
	xdata signed char c;
	unsigned char xdata * xdata tmp;
	
	tmp = calloc(LEN_60, sizeof(char));
	...
	if(c != -1) { // Variable gefunden
		...
		if(vr.nr[zeile] >= DIR_VAR) { // Verzeichnis
			...
			if(vr.nr[zeile] == ip_en) {
				sprintf(tmp, "%bu.%bu.%bu.%bu", ip_adr.b[0], ip_adr.b[1], ip_adr.b[2], ip_adr.b[3]);
				...
				//~ else if(km==4)
					...
					sprintf(tmp, " %5.2f", (float)vek[vr.nr[zeile]].i / 100.0);
				else if(km==1)
					sprintf(tmp, " %5.1f", (float)vek[vr.nr[zeile]].i / 10.0);
				else if(km==62)
					sprintf(tmp, " %7.2f", (float)vek[vr.nr[zeile]].i / 100.0);
				else {
					if(vr.nr[zeile] == softver_ix)
						sprintf(tmp, " %4d %4d", menu_ver.ui, vek[vr.nr[zeile]].i);
					else
						sprintf(tmp, " %5d", vek[vr.nr[zeile]].i);
				}
			} 
		}
		add_var(zeile, tmp);
...
unsigned char menu_fuellen(void) {
	...
	len_128k = BUF_LEN; // sizeof(rram.buf_tmp);
	//~ #define rram.tmp_buf rram.tmp_buf
	...
	for(row = 0,i = 0; row < MENU_LENGTH; ++row) {
		...
		text_nr = rram.tmp_buf[row*2];
		text_nr <<= 8;
		//~ text_nr |= rram.buf_tmp[row*2 + 1];
		text_nr |= rram.tmp_buf[row*2 + 1];
		if(text_nr) {	
			if(text_ok(text_nr))
				menu_vec[i++] = text_nr;
		}
	...
	rram.tmp_buf[0] = menu_stack.txt_nr[0] - menu_stack.pos[0] + 1;
	for(row = 1; row < 8; ++row) {
		text_nr = menu_vec[rram.tmp_buf[0] + row - 1];
		if(text_nr) {	
...
void cursor_nach_unten(void) {	
	...
	++menu_stack.pos[0];
	++menu_stack.txt_nr[0];
	#define text_nr_c menu_vec[menu_stack.txt_nr[0]]
	...
	if(text_nr_c) {
		...
		if(menu_stack.pos[0] > 7) {	
			unsigned char i;
			for(i = 1; i < 8; i++) {	
				memcpy(txt_dflt[i] + 12, txt_dflt[i + 1] + 12,sizeof(txt_dflt[i]) - 11);
			}
...
void cursor_nach_oben(void) {// Cursor nach oben
	if(menu_stack.txt_nr[0] > 1) {	
		...
		if(menu_stack.pos[0] < 1) {
			...
			set_nmbr_txt(text_nr_c);
			for(i = 7; i > 1; --i) {	
				memcpy(txt_dflt[i] + 11, txt_dflt[i - 1] + 11, sizeof(txt_dflt[i]) - 11);
			}
				...
				if(ON_OFF){
					...
					if(MENU_MODE == 11)	{
						...
						if(vek[vr.nr[menu_stack.pos[0]]].ui != var_wert) {	
							if(vr.nr[menu_stack.pos[0]] <= code0_x || vr.nr[menu_stack.pos[0]]==komm_t) {
...
bit txt_var_fn(unsigned int v) {
	if((vr.nr[v] >= btr_art_t && vr.nr[v] <= set_nt) || vr.nr[v] == komm_t || vr.nr[v] == wreset_en)
		return 1;
	else 
		return 0;	
}
...
void menu_rt(void) {
	...
	xdata unsigned char tmp, 
		...
		memset(&rram,0,sizeof(rram));
		buf_d = calloc (LEN_60, sizeof(char));           
				...
				if(vr.pos[menu_stack.pos[0]] <= strlen(txt_dflt[menu_stack.pos[0]])) { // nur wenn Variable existiert 
					if(vr.nr[menu_stack.pos[0]] >= DIR_VAR) {// Untermenu
						...
						MENU_MODE = 1;
							...
							vr.nr[menu_stack.pos[0]] == komm_t    || vr.nr[menu_stack.pos[0]] == wreset_en) {
						// Wert verstellen
		...
		sprintf(txt_dflt[Q_prc],"%s%4.1f L/h, %3u", basic[Q_prc],fl,lev);
				...
				}	
				{
					...
					if(tmp) {
						...
						txt_anf(tmp);
						sprintf(txt_dflt[sts_line],"%s%bu+%bu ", basic[sts_line], status_zeile.ctr_alr, status_zeile.ctr_warn);
						//~ read_text(txt_dflt[sts_line]);
						memset(buf_d, 0, LEN_60);
						read_text(buf_d);
						strcat(txt_dflt[sts_line], buf_d);
...
bit add_prc(unsigned char line, unsigned char med) {
	...
	b = 0;
	len = strlen(txt_dflt[line]);
	if(rl[0].ind == med) {
		sprintf(txt_dflt[line] + len, ",%bu%%", rl[0].prc);
		//~ strcat(txt_dflt[line],"%");
		b = 1;
	} else if(rl[1].ind == med) {
		sprintf(txt_dflt[line] + len, ",%bu%%", rl[1].prc);
		//~ strcat(txt_dflt[line], "%");
		b = 1;
	}
	return b;
...
void tank_lev(unsigned char col, unsigned char row, unsigned char lev) {
	...
	unsigned char xdata * xdata str_tl;
	str_tl = calloc(5, sizeof(char));
	#define x_tl str_tl[3]
	#define b_tl str_tl[4]
	
	line_v(col,    row,    "46");
	line_v(col+16, row,    "46");
	line_h(col,    row,    "16");
	line_h(col,    row+46, "17");
	for(x_tl = 0; x_tl < 9; ++x_tl) {
		b_tl = 46 + row - lev;
		str_tl[0] = 0x30 + lev/10;
		str_tl[1] = 0x30 + lev%10;
		str_tl[2] = 0;
		line_v(col + 4 + x_tl, b_tl, str_tl);
	}
...
void strom_lev(unsigned char col, unsigned char row, unsigned int prc) {
	...
	if(prc > 150) 
		prc = 150;
	b_tl = prc * 0.4;
	for(x_tl = 1; x_tl < 6; ++x_tl) {
		str_tl[0] = 0x30 + b_tl/10;
		str_tl[1] = 0x30 + b_tl%10;
		str_tl[2] = 0;
		line_h(col, row + x_tl, str_tl);
	}
	free(str_tl);
}
...
void show_update(void) {
	static char chr;
	static char cnt = 0;
	static unsigned long adr_tmp = 0;
	...
	sprintf(txt_dflt[0] + strlen(txt_dflt[0]), " UPDATE: %c %lu %bu", chr, adr_128k, empf1.cnt);

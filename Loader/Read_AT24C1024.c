...
void update(void)
{
	...
	eeprom_test(2); 	// 2 - flash eeprom
	if(!crc)
	{
		if(flash) // programmieren
		{
			test_ergebnis(0);
			eprm = 1;
		}
	...
	// ----------  Menutest -----------------	
	message(menu_txt,0);
	eeprom_test(1); 	// 1 - interner EEPROM;
	test_ergebnis((bit)crc);
	message(update_txt, 2);
	eeprom_test(0);	// 0 - udate_menu;
	test_ergebnis((bit)crc);
	if(!crc)
	{
		xdata unsigned long zlr;
		message(program_txt, 2);
		for(adr_128k = 0, zlr = 0, adr_wr = 1024; zlr < len.ul; ) 
		{
			len_128k = 128;
			memset(&buf,0,sizeof(buf));
			// for(page_cnt = 0; page_cnt < 4; ++page_cnt)
			for(page_cnt = 0; page_cnt < 2; ++page_cnt)
			{	
				ptr_128k = buf.eepr_r[page_cnt]; 
				ee_rdseq_128k(0); // 0 - udate_menu;
				zlr += 128;
			}
			len_128k  = 256; // 512;
...
/*	
	wr = ee_rdseq_128k(0); // 0 - udate_menu;
	if(wr == 1)
	{
		test_ergebnis((bit)crc);
	}
